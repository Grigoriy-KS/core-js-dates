...
function getNextFriday(date) {
  ...
  const numberOfDaysUntilFriday =
    currentDayOfWeek < 5 ? 5 - currentDayOfWeek : 7 - (currentDayOfWeek - 5);
  return new Date(date.setDate(date.getDate() + numberOfDaysUntilFriday));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (
    Date.parse(dateEnd) / (24 * 60 * 60 * 1000) -
    Date.parse(dateStart) / (24 * 60 * 60 * 1000) +
    1
  );
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    dateInMilliseconds >= Date.parse(period.start) &&
    dateInMilliseconds <= Date.parse(period.end)
  );
}
...
function getWeekNumberByDate(date) {
  ...
  function findFirstMonday(dateForFunction) {
    let firstMonday = new Date(dateForFunction.getFullYear(), 0, 1);
    switch (firstMonday.getDay()) {
      case 0:
      ...
      case 3:
      case 4:
        firstMonday = firstMonday.setDate(
          firstMonday.getDate() + (1 - firstMonday.getDay())
        );
        break;
      case 5:
      case 6:
        firstMonday = firstMonday.setDate(
          firstMonday.getDate() + (8 - firstMonday.getDay())
        );
        break;
      default:
        break;
  ...
  const startLastYear = new Date(new Date(givenDate.getFullYear() - 1, 0, 1));

  if (givenDate > findFirstMonday(givenDate)) {
    weekNumber = Math.ceil(
      ((givenDate - findFirstMonday(givenDate)) / 86400000 + 1) / 7
    );
  } else {
    weekNumber = Math.ceil(
      ((givenDate - findFirstMonday(startLastYear)) / 86400000 + 1) / 7
    );
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const periodStartArray = period.start
    .split('-')
    .map((element) => Number(element));
  const periodEndArray = period.end
    .split('-')
    .map((element) => Number(element));
  ...
  const tempDate = new Date(
    periodStartArray[2],
    periodStartArray[1] - 1,
    periodStartArray[0]
  );
  const workSchedule = [];
  let counter = 1;
  while (tempDate.getTime() <= periodEndDate.getTime()) {
    if (counter <= countWorkDays) {
      workSchedule.push(
        Intl.DateTimeFormat('ru', { dateStyle: 'short' })
          .format(tempDate)
          .replaceAll('.', '-')
      );
    }
